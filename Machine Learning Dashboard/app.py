    ...
    if not st.session_state.target_selected:
        ...
        with col1:
            handle_missing = st.checkbox(
                ...
                help="Fill missing values: numeric columns with mean, categorical with mode"
            )
            
            encode_categorical = st.checkbox(
                "Encode Categorical Variables",
                help="Convert categorical variables to numeric using label encoding"
            )
        
        with col2:
            scale_features = st.checkbox(
                "Scale Features",
                help="Normalize feature values to improve model performance"
            )
